...
function home() {
  ...
  Object.values(questionSet).map((s) => {
    ...
    <div class="set-box" id = "boxex" onclick="listOfQuestions(${s.id})">
...
function listOfQuestions(id) {
  ...
  setOfQuestions.classList.remove("d-none");
  document.querySelector("#workspace").classList.add("d-none");
  setOfQuestions.innerHTML = `<h2 class="mb-4"><span>${set.title}</span><button class="btn" style="background:#303041; margin-left:13px" onclick="home()"><i class="fa fa-arrow-left" aria-hidden="true"></i></button></h2>`;
...
function submit() {
  ...
  for (let i = 0; i < check.length; i++) {
    ...
    if (JSON.stringify(answer) == JSON.stringify(answers[i])) {
      trueAnswer++;
      show.innerHTML += `<p class="d-flex justify-content-between align-items-center flex-wrap">${functionName}(${check[i]}) => ${answers[i]} <button class="btn bg-success answer__btn ">Javobingiz: ${answer}</button></p>`;
    } else {
      show.innerHTML += `<p class="d-flex justify-content-between align-items-center flex-wrap">${functionName}(${check[i]}) => ${answers[i]} <button class="btn bg-danger answer__btn ">Javobingiz: ${answer}</button></p>`;
    }
  }
...
const lightToggle = document.querySelector("#ligth-toggle");
...
lightToggle.addEventListener("click", function () {
  ...
  let setOfQuestions = document.getElementById("setOfQuestions");
  setOfQuestions.classList.remove("dark");
  setOfQuestions.classList.add("light");
...
darkToggle.addEventListener("click", function () {
  ...
  setBoxes.forEach((box) => {
    box.style.backgroundColor = "#303041";
  });
  let setOfQuestions = document.getElementById("setOfQuestions");
  setOfQuestions.classList.remove("light");
  setOfQuestions.classList.add("dark");
